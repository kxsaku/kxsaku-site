...
serve(async (req) => {
  ...
  try {
    ...
    if (!nextText) return json({ error: "Missing body" }, 400);
    ...
    if (msg.sender_role !== "client") return json({ error: "Only client messages can be edited" }, 403);
    if (msg.deleted_at) return json({ error: "Deleted messages cannot be edited" }, 409);
